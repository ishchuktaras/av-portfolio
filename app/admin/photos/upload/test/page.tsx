...
import { useAuth } from "@/components/auth-provider" // Updated import
...
export default function TestUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setSuccess(true)
      setUploadedImage(result.downloadUrl)
              ...
              <Alert className="mb-4 bg-primary/10 border-primary text-primary">
                <AlertDescription>
                  ...
                  {uploadedImage && uploadedImage.startsWith("data:") && (
                    <div className="mt-2 text-xs">(Running in demo mode - image stored locally)</div>
                  )}
                </AlertDescription>
              </Alert>
            )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {uploadedImage ? "Uploaded Image" : "Preview"}
            </CardTitle>
          </CardHeader>
          <CardContent>
